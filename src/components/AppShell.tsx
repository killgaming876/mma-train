...
export function MagneticButton({ children, onClick, variant = 'primary', type = 'button' }: { children: ReactNode; onClick?: () => void; variant?: 'primary' | 'ghost'; type?: 'button' | 'submit' }) {
  return <motion.button type={type} className={`magnetic-button ${variant}`} whileHover={{ y: -3, scale: 1.018, rotateX: 1.5 }} whileTap={{ scale: 0.97 }} onClick={onClick}>{children}<span className="button-arrow">↗</span></motion.button>;
}
